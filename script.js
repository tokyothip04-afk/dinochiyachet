...
const GRAVITY = 3000;
const JUMP_V = 1000;
const JUMP_MIN = 800;       // tap jump strength (tweak)
...
// Drunk
const DRUNK_FALL_CHANCE = 0.33;      // % of drunk enemies will fall
...
function gameOver() {
  ...
  if (score > best) {
    ...
    bestEl.textContent = best;
    document.getElementById("title").textContent = "เข้าใกล้พี่ทานตะวันมากขึ้น 😚";
...
function addObstacle({ type, bottom, speedMult = 1 }) {
  ...
  if (type === "dog") {
    el.classList.add("enemyBob");
    el.classList.add("flip");   // ← this flips the sprite
  }
